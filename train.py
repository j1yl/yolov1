...
def main():
    ...
    train_dataset = VOCDataset(
        "data/VOC2012", split="train", transform=transform, S=7, B=2, C=20
    )

    val_dataset = VOCDataset(
        "data/VOC2012", split="val", transform=transform, S=7, B=2, C=20
    )
    ...
    for epoch in range(EPOCHS):
        ...
        if (epoch + 1) % 9 == 0:
            ...
            torch.save(
                checkpoint, os.path.join("checkpoints", f"yolov1_epoch_{epoch+1}.pt")
            )
    ...
    print(
        f"Training completed in {total_time:.2f} seconds ({total_time/3600:.2f} hours)"
    )
